    ...
    for i_surpop in [5,7]: #nb de survie 4 et 6
        for i_naiss in [1,2]: #nb pour naissance 2 et 3
            for i_prop in range(0,8):
                sns.distplot(a[i_surv, i_surpop,i_naiss,3,:], hist = 0, color =(random(),random(),random()), axlabel = f'proportion pour tout i_prop avec i_surv = {i_surv+1} i_surpop = {i_surpop-1} et i_naiss = {i_naiss+1}', label = f'i_surpop = {i_surpop}')
            plt.legend(loc = 0)
            plt.show()
